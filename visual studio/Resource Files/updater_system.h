...
{
	DWORD timeout = 2000; // Slightly higher timeout for API calls

	HINTERNET hInternet = InternetOpen(L"Spencer-Macro-Utilities-Updater",
	...
	// Added INTERNET_FLAG_SECURE because GitHub API requires HTTPS
...
static std::string GetRemoteVersion()
{
	...
	std::string apiResponse = GetStringFromUrl(
		L"https://api.github.com/repos/Spencer0187/Spencer-Macro-Utilities/releases/latest");
	if (apiResponse.empty())
		return "";

	try {
		auto j = json::parse(apiResponse);
		std::string tag = j["tag_name"]; // e.g. "V3.1.1"

		// Remove 'V' or 'v' prefix if it exists to match the local "3.1.1" format
...
// Find the download URL for the .zip asset from the GitHub API
static std::string GetRemoteZipUrl()
{
	std::string apiResponse = GetStringFromUrl(
		L"https://api.github.com/repos/Spencer0187/Spencer-Macro-Utilities/releases/latest");
	if (apiResponse.empty())
		...
		if (j.contains("assets") && j["assets"].is_array()) {
			for (const auto &asset : j["assets"]) {
				...
				if (assetName.length() > 4 &&
				    assetName.substr(assetName.length() - 4) == ".zip") {
...
bool DownloadToMemory(const std::wstring &url, std::vector<char> &data)
{
	...
	// Added INTERNET_FLAG_SECURE for HTTPS downloads
	HINTERNET hConnect = InternetOpenUrl(
...
bool ExtractFileFromMemory(const std::vector<char> &zipBuffer, const std::string &fileNameToExtract,
			   std::vector<char> &extractedData)
{
	...
	int file_index =
		mz_zip_reader_locate_file(&zip_archive, fileNameToExtract.c_str(), NULL, 0);
	if (file_index < 0) {
	...
	void *pBuffer =
		mz_zip_reader_extract_to_heap(&zip_archive, file_index, &uncompressed_size, 0);
	if (!pBuffer) {
		mz_zip_reader_end(&zip_archive);
		return false; // Failed to extract
	}

	extractedData.assign(static_cast<char *>(pBuffer),
			     static_cast<char *>(pBuffer) + uncompressed_size);
...
{
	// 1. Get remote ZIP update URL from GitHub API
	std::string downloadUrlAnsi = GetRemoteZipUrl();
	if (downloadUrlAnsi.empty()) {
		MessageBox(NULL, L"Failed to retrieve update URL from GitHub assets.",
			   L"Update Error", MB_OK | MB_ICONERROR);
		return;
	}

	std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;
	std::wstring downloadUrl = converter.from_bytes(downloadUrlAnsi);

	// 2. Download the zip file into a memory buffer
	std::vector<char> zipData;
	if (!DownloadToMemory(downloadUrl, zipData)) {
		MessageBox(NULL,
			   L"Failed to download the update. Please check your internet connection.",
			   L"Update Error", MB_OK | MB_ICONERROR);
		return;
	}
	...
	if (!ExtractFileFromMemory(zipData, exeNameInZip, exeData)) {
		MessageBox(
			NULL,
			L"Failed to extract the update from the downloaded package. It may be corrupt.",
			L"Update Error", MB_OK | MB_ICONERROR);
		return;
	}
	...
	if (hFile == INVALID_HANDLE_VALUE) {
		MessageBox(
			NULL,
			L"Could not create temporary update file. Please check folder permissions.",
			L"Update Error", MB_OK | MB_ICONERROR);
		return;
	}
	DWORD bytesWritten;
	if (!WriteFile(hFile, exeData.data(), (DWORD)exeData.size(), &bytesWritten, NULL) ||
	    bytesWritten != exeData.size()) {
		CloseHandle(hFile);
		DeleteFileW(tempExePath.c_str());
		MessageBox(NULL, L"Failed to write update data to temporary file.", L"Update Error",
			   MB_OK | MB_ICONERROR);
		return;
	}
	CloseHandle(hFile);
	...
	if (!currentFolderName.empty() && currentFolderName.length() > wLocalVersion.length() &&
	    currentFolderName.substr(currentFolderName.length() - wLocalVersion.length()) ==
		    wLocalVersion) {
		// Check for a common separator before the version string to be safer
		wchar_t separator =
			currentFolderName[currentFolderName.length() - wLocalVersion.length() - 1];
		if (separator == L'-' || separator == L'_' || separator == L' ' ||
		    separator == 'V' || separator == 'v') {
	...
	std::wstring batchFilePath =
		std::wstring(tempDir) + L"updater-" + GenerateRandomHexString() + L".bat";

	if (shouldRenameFolder) {
		// --- BATCH SCRIPT GENERATION: WITH FOLDER RENAME ---
		std::wstring newFolderName =
			currentFolderName.substr(0, currentFolderName.length() -
							    wLocalVersion.length()) +
			wNewVersion;
		...
		std::wstring newFolderPath =
			std::wstring(parentOfCurrentArr) + L"\\" + newFolderName;
		std::wstring newExePathInRenamedFolder = newFolderPath + L"\\" + currentExeName;

		std::wstring tempExePathAfterRename =
			newFolderPath + L"\\" + randomFileName + L".tmp";
		...
		batchScriptContent =
			...
			L"move \"" +
			currentFolderPath + L"\" \"" + newFolderPath +
			L"\"\n\n"
			// 2. Delete the old executable (which is now at its new path).
			L"del /F /Q \"" +
			finalExePathInNewFolder +
			L"\"\n"
			// Rename the .tmp file to the final executable name.
			L"move \"" +
			tempExePathAfterRename + L"\" \"" + finalExePathInNewFolder +
			L"\"\n\n"
	...
	HANDLE hBatchFile = CreateFileW(batchFilePath.c_str(), GENERIC_WRITE, 0, NULL,
	...
	if (hBatchFile != INVALID_HANDLE_VALUE) {
		...
		WriteFile(hBatchFile, mbBatchScript.c_str(), (DWORD)mbBatchScript.length(),
			  &batchBytesWritten, NULL);
		...
		if (!shouldRenameFolder) {
			params = L"\"" + currentExePath + L"\" \"" + tempExePath + L"\" \"" +
				 currentExeName + L"\"";
		}
		...
		MessageBox(NULL, L"Could not create the updater script. Please check permissions.",
			   L"Update Error", MB_OK | MB_ICONERROR);
		DeleteFileW(tempExePath.c_str());
	}
}