...
CGEventRef event_tap_callback(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void* refcon) {
    if (type == kCGEventKeyDown || type == kCGEventKeyUp) {
        ...
        if (vk_code != VK_UNASSIGNED) {
            ...
            // Check if event came from our event source (program-generated)
            CGEventSourceRef source = CGEventCreateSourceFromEvent(event);
            bool from_program = (source != nullptr && CFEqual(source, event_source));
            if (source) CFRelease(source);
            
            // Handle space key for bhop
            if (vk_code == 0x20) {
                if (is_down && !from_program) {
                    // Real user pressed space - send keyup and start bhop
                    printf("[Helper] Real space press detected - starting bhop\n");
                    emit_key_event(0x31, false); // Send space up
                    isbhop = true;
                } else if (!is_down && !from_program) {
                    // Real user released space - stop bhop
                    printf("[Helper] Real space release detected - stopping bhop\n");
                    isbhop = false;
                }
                // Don't update shared state for space during bhop
                if (!isbhop) {
