	...
	if (nCode == HC_ACTION) {
		...
		if (pkbhs->vkCode == vk_bunnyhopkey) {
			if ((pkbhs->flags & LLKHF_INJECTED) == 0) {
				...
				g_isVk_BunnyhopHeldDown.store((wParam & 1) == 0,
...
bool IsWheelUp()
{
	return g_mouseWheel.IsWheelUp();
}
bool IsWheelDown()
{
	return g_mouseWheel.IsWheelDown();
}
		...
		if (hProcess) {
			...
			if (GetProcessTimes(hProcess, &ftCreation, &ftExit, &ftKernel, &ftUser)) {
				ULONGLONG creationTime =
					(static_cast<ULONGLONG>(ftCreation.dwHighDateTime) << 32) |
...
static std::array<SectionConfig, section_amounts> SECTION_CONFIGS = {
	{{"Freeze", "Automatically Tab Glitch With a Button"},
	 ...
	 {"Helicopter High Jump",
	  "Use COM Offset to Catapult Yourself Into The Air by Aligning your Back Angled to the Wall and Jumping and Letting Your Character Turn"},
	 {"Speedglitch", "Use COM offset to Massively Increase Midair Speed"},
	 {"Item Unequip COM Offset",
	  "Automatically Do a /e dance2 Item COM Offset Where You Unequip the Item"},
	 {"Press a Button",
	  "Whenever You Press Your Keybind, it Presses the Other Button for One Frame"},
	 {"Wallhop/Rotation", "Automatically Flick and Jump to easily Wallhop On All FPS"},
	 {"Walless LHJ",
	  "Lag High Jump Without a Wall by Offsetting COM Downwards or to the Right"},
...
static bool IsHotkeyPressed(unsigned int combinedKey)
{
	...
	if ((combinedKey & HOTKEY_MASK_WIN) && !(IsKeyPressed(VK_LWIN) || IsKeyPressed(VK_RWIN)))
		return false;

	if ((combinedKey & HOTKEY_MASK_CTRL) && !IsKeyPressed(VK_CONTROL))
		return false;
	if ((combinedKey & HOTKEY_MASK_ALT) && !IsKeyPressed(VK_MENU))
		return false;
	if ((combinedKey & HOTKEY_MASK_SHIFT) && !IsKeyPressed(VK_SHIFT))
		return false;
...
static void FormatHexKeyString(unsigned int combinedCode, char *buffer, size_t size)
{
	...
	if (combinedCode & HOTKEY_MASK_WIN)
		hexStr += "0x5B + ";
	if (combinedCode & HOTKEY_MASK_CTRL)
		hexStr += "0x11 + ";
	if (combinedCode & HOTKEY_MASK_ALT)
		hexStr += "0x12 + ";
	if (combinedCode & HOTKEY_MASK_SHIFT)
		hexStr += "0x10 + ";
...
static void GetKeyNameFromHex(unsigned int combinedKeyCode, char *buffer, size_t bufferSize)
{
	...
	std::string prefix = "";
	if (combinedKeyCode & HOTKEY_MASK_WIN)
		prefix += "Win + ";
	if (combinedKeyCode & HOTKEY_MASK_CTRL)
		prefix += "Ctrl + ";
	if (combinedKeyCode & HOTKEY_MASK_ALT)
		prefix += "Alt + ";
	if (combinedKeyCode & HOTKEY_MASK_SHIFT)
		prefix += "Shift + ";
...
static unsigned int BindKeyMode(unsigned int *keyVar, unsigned int currentkey, int selected_section)
{
	...
	if (state.bindingMode) {
		...
		if (IsKeyPressed(VK_LWIN) || IsKeyPressed(VK_RWIN))
			currentModifiers |= HOTKEY_MASK_WIN;
		if (IsKeyPressed(VK_CONTROL))
			currentModifiers |= HOTKEY_MASK_CTRL;
		if (IsKeyPressed(VK_MENU))
			currentModifiers |= HOTKEY_MASK_ALT;
		if (IsKeyPressed(VK_SHIFT))
			currentModifiers |= HOTKEY_MASK_SHIFT;

		// 2. Generate Live Preview Strings (Win -> Ctrl -> Alt -> Shift)
		state.buttonText = "Press a Key...";

		std::string previewHuman = "";
		if (currentModifiers & HOTKEY_MASK_WIN)
			previewHuman += "Win + ";
		if (currentModifiers & HOTKEY_MASK_CTRL)
			previewHuman += "Ctrl + ";
		if (currentModifiers & HOTKEY_MASK_ALT)
			previewHuman += "Alt + ";
		if (currentModifiers & HOTKEY_MASK_SHIFT)
			previewHuman += "Shift + ";
		...
		// 3. Update buffers
		strncpy(state.keyBufferHuman, previewHuman.c_str(),
			sizeof(state.keyBufferHuman) - 1);
		...
		for (int key = 1; key < 258; key++) {
			...
			if (currentlyPressed && !state.keyWasPressed[key]) {
				...
				GetKeyNameFromHex(finalCombo, state.keyBufferHuman,
						  sizeof(state.keyBufferHuman));
				FormatHexKeyString(finalCombo, state.keyBuffer,
						   sizeof(state.keyBuffer));
		...
		static const int modifierKeys[] = {VK_LWIN,  VK_RWIN,  VK_LCONTROL, VK_RCONTROL,
...
void CheckDisplayScale(HWND hwnd, int display_scale)
{
	...
	if (currentScalePercent != 100 && display_scale != currentScalePercent) {
		std::wstring msg =
			L"Your display scaling doesn't match the program's settings. "
			L"Your current display scale is " +
			std::to_wstring(currentScalePercent) + L"%, the Macro's display scale is " +
			std::to_wstring(display_scale) +
			L"%. Make these two equal by either updating the Macro's Settings or your Windows Settings.";

		MessageBox(hwnd, msg.c_str(), L"Display Scaling Mismatch", MB_OK | MB_ICONWARNING);
	}
}
...
static void RunGUI()
{
	...
	RegisterClassEx(&wc);
	HWND hwnd = CreateWindow(wc.lpszClassName, _T("Spencer Macro Client"), WS_OVERLAPPEDWINDOW,
	...
	if (WindowPosX == 0 && WindowPosY == 0) {
		...
		SetWindowPos(hwnd, NULL, WindowPosX, WindowPosY, screen_width, screen_height,
			     SWP_NOZORDER);
	}
...
#define SAFE_CONVERT_DOUBLE(var, src) \
	...
	while (running) {
		...
		if (currentTime >= nextFrameTime) {
			...
			if (ImGui::BeginPopupModal("Administrator Required", &bShowAdminPopup,
						   ImGuiWindowFlags_AlwaysAutoResize)) {
				ImGui::Text("WinDivert features require Administrator privileges.");
				ImGui::Text("This process involves:");
				ImGui::BulletText(
					"Extracting SMCWinDivert.dll and WinDivert64.sys to the current folder.");
			...
			ImGui::BeginChild("GlobalSettings",
					  ImVec2(display_size.x - 16, settings_panel_height), true);
			...
			ImGui::TextWrapped(
				"DISCLAIMER: THIS IS NOT A CHEAT, IT NEVER INTERACTS WITH ROBLOX MEMORY.");

			// Macro Toggle Checkbox
			ImGui::PushStyleColor(ImGuiCol_Text,
					      macrotoggled ? GetCurrentTheme().success_color
							   : GetCurrentTheme().error_color);
			ImGui::AlignTextToFramePadding();
			ImGui::Checkbox("Macro Toggle (Anti-AFK remains!)",
					&macrotoggled); // Checkbox for toggling
			ImGui::PopStyleColor();
			...
			ImGui::AlignTextToFramePadding();
			ImGui::TextWrapped(
				g_isLinuxWine ? "Roblox Executable Name/PIDs (Space Separated):"
					      : "Roblox Executable Name:");
			ImGui::SameLine();
			ImGui::SetNextItemWidth(250.0f);

			ImGui::InputText("##SettingsTextbox", settingsBuffer,
					 sizeof(settingsBuffer),
					 g_isLinuxWine ? 0 : ImGuiInputTextFlags_CharsNoBlank);

			// Button to reset Roblox EXE name
			ImGui::SameLine();
			if (ImGui::Button("R", ImVec2(25, 0))) {
				if (!g_isLinuxWine) {
					std::snprintf(settingsBuffer, sizeof(settingsBuffer),
			...
			ImU32 color = processFound ? ImGui::ColorConvertFloat4ToU32(
							     GetCurrentTheme().success_color)
			...
			ImGui::SetCursorScreenPos(
				ImVec2(tooltipcursorpos.x + TextSizeCalc.x, tooltipcursorpos.y));
			...
			if (ImGui::InputText("##Sens", RobloxSensValue, sizeof(RobloxSensValue),
				...
				if (sensValue != 0.0f) {
					float pixels = std::atof(WallhopDegrees) *
						       (camfixtoggle ? 1000.0f : 720.0f) /
						       (360.0f * sensValue);
					snprintf(WallhopPixels, sizeof(WallhopPixels), "%.0f",
						 pixels);
					try {
			...
			if (ImGui::Checkbox("##CamfixToggle", &camfixtoggle) ||
			    PreviousSensValue == -1) {
				wallhopupdate = false;
				if (PreviousSensValue !=
				    -1) { // Localize wallhop updates only to cam-fix
					wallhopupdate = true;
				}
				PreviousSensValue = -1;
				...
				try {
					if (wallhopupdate) {
						float factor = camfixtoggle ? 1.388888889f
									    : 1.0f / 1.388888889f;
						if (wallhopswitch) {
							wallhop_dx = std::round(
								std::stoi(WallhopPixels) *
								(camfixtoggle ? -factor : factor));
							wallhop_dy = std::round(
								std::stoi(WallhopPixels) *
								(camfixtoggle ? factor : -factor));
						} else {
							wallhop_dx = std::round(
								std::stoi(WallhopPixels) * factor);
							wallhop_dy = std::round(
								std::stoi(WallhopPixels) * -factor);
				...
				wallwalk_strengthx = -static_cast<int>(
					std::round((baseValue / CurrentWallWalkValue) * 0.13f));
				wallwalk_strengthy = static_cast<int>(
					std::round((baseValue / CurrentWallWalkValue) * 0.13f));
				...
				try {
					...
					RobloxPixelValue = static_cast<int>(std::round(
						(baseValue / CurrentSensValue) * multiplier));
				} catch (const std::invalid_argument &) {
				} catch (const std::out_of_range &) {
				}
			...
			if (show_settings_menu ? ImGui::Button("Settings <-")
			...
			if (show_settings_menu) {
				...
				ImVec2 child_pos =
					ImVec2((main_window_size.x * 0.4f),
				...
				if (ImGui::Begin("Settings Menu", &show_settings_menu,
						 window_flags)) {
					...
					ImGui::Text(
						"Your Current Windows Display Scale Value (10-500%):");
					...
					if (ImGui::Button((shiftState.buttonText + "##ShiftKey")
								  .c_str())) {
					...
					ImGui::SameLine();
					vk_shiftkey = BindKeyMode(&vk_shiftkey, vk_shiftkey,
								  selected_section);

					// Human readable display
					ImGui::SetNextItemWidth(150.0f);
					GetKeyNameFromHex(vk_shiftkey, shiftState.keyBufferHuman,
							  sizeof(shiftState.keyBufferHuman));
					ImGui::InputText("##ShiftKeyHuman",
							 shiftState.keyBufferHuman,
							 sizeof(shiftState.keyBufferHuman),
					...
					ImGui::SameLine();
					ImGui::InputText(
						"##ShiftKeyHex", shiftState.keyBuffer,
						sizeof(shiftState.keyBuffer),
						ImGuiInputTextFlags_CharsNoBlank |
					...
					ImGui::Checkbox(
						"Force-Set Chat Open Key to \"/\" (Most Stable)",
						&chatoverride);
					...
					ImGui::TextWrapped(
						"Custom Chat Key (Must disable Force-Set):");

					// Bind button
					if (ImGui::Button(
						    (chatState.buttonText + "##ChatKey").c_str())) {
					...
					ImGui::SameLine();
					vk_chatkey = BindKeyMode(&vk_chatkey, vk_chatkey,
								 selected_section);

					// Human readable display
					ImGui::SetNextItemWidth(150.0f);
					GetKeyNameFromHex(vk_chatkey, chatState.keyBufferHuman,
							  sizeof(chatState.keyBufferHuman));
					ImGui::InputText("##ChatKeyHuman", chatState.keyBufferHuman,
							 sizeof(chatState.keyBufferHuman),
							 ImGuiInputTextFlags_ReadOnly);
					ImGui::SameLine();
					ImGui::TextWrapped("Key Binding");
					...
					ImGui::InputText(
						"##ChatKeyHex", chatState.keyBuffer,
						sizeof(chatState.keyBuffer),
						ImGuiInputTextFlags_CharsNoBlank |
							ImGuiInputTextFlags_CharsHexadecimal |
							ImGuiInputTextFlags_ReadOnly);
					ImGui::PopID();
					...
					ImGui::TextWrapped(
						"Old Unicode Chat-Typing (Works across languages but may be blocked by some anticheats)");

					ImGui::Separator();

					ImGui::AlignTextToFramePadding();
					ImGui::TextWrapped(
						"Delay between every key press in chat (ms):");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(30.0f);
					if (ImGui::InputText(
						    "##PasteDelay", PasteDelayChar,
						    sizeof(PasteDelayChar),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
					...
					ImGui::TextWrapped(
						"Custom Anti-AFK Key (That the macro uses):");

					// Bind button
					if (ImGui::Button(
						    (afkState.buttonText + "##Afkkey").c_str())) {
					...
					ImGui::SameLine();
					vk_afkkey = BindKeyMode(&vk_afkkey, vk_afkkey,
								selected_section);

					// Human readable display
					ImGui::SetNextItemWidth(150.0f);
					GetKeyNameFromHex(vk_afkkey, afkState.keyBufferHuman,
							  sizeof(afkState.keyBufferHuman));
					ImGui::InputText("##AFKKeyHuman", afkState.keyBufferHuman,
							 sizeof(afkState.keyBufferHuman),
							 ImGuiInputTextFlags_ReadOnly);
					ImGui::SameLine();
					ImGui::TextWrapped("Key Binding");
					...
					ImGui::InputText(
						"##AfkkeyHex", afkState.keyBuffer,
						sizeof(afkState.keyBuffer),
						ImGuiInputTextFlags_CharsNoBlank |
							ImGuiInputTextFlags_CharsHexadecimal |
							ImGuiInputTextFlags_ReadOnly);
					ImGui::PopID();
					...
					ImGui::SetNextItemWidth(30.0f);
					if (ImGui::InputText(
						    "##AntiAFKTime", AntiAFKTimeChar,
						    sizeof(AntiAFKTimeChar),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
					...
					ImGui::Checkbox("Replace shiftlock with zooming in",
							&globalzoomin);
					ImGui::SameLine();
					...
					ImGui::Checkbox("Double-Press AFK keybind during Anti-AFK",
							&doublepressafkkey);

					ImGui::Separator();

					if (ImGui::Checkbox("Remove Side-Bar Macro Descriptions",
							    &shortdescriptions)) {
						InitializeSections();
					}
					...
					ImGui::Dummy(
						ImVec2(0.0f, ImGui::GetTextLineHeight() * 0.5f));

					ImGui::PushStyleColor(ImGuiCol_Text,
							      GetCurrentTheme().accent_primary);
					...
					if (ImGui::IsItemHovered()) {
						...
						if (ImGui::IsItemClicked()) {
							ShellExecuteA(
								NULL, "open",
								"https://github.com/sponsors/Spencer0187",
								NULL, NULL, SW_SHOWNORMAL);
						}
					}
					ImGui::PopStyleColor();
					...
					ImGui::Text("%s",
						    "Github Doesn't take any of the profits.");
			...
			if (show_theme_menu ? ImGui::Button("Theme Editor <-")
					    : ImGui::Button("Theme Editor ->")) {
			...
			ImGui::BeginChild("LeftScrollSection",
					  ImVec2(left_panel_width, ImGui::GetWindowSize().y -
									   settings_panel_height -
									   20),
					  true);

			for (size_t display_index = 0; display_index < section_amounts;
			     ++display_index) {

				int i = section_order
					[display_index]; // Get section index from order array

				ImGui::PushID(i);

				float buttonWidth =
					left_panel_width - ImGui::GetStyle().FramePadding.x * 2;
				...
				auto Brighten = [](ImVec4 col, float factor) -> ImVec4 {
					return ImVec4(std::min(col.x * factor, 1.0f),
						      ...
						      std::min(col.z * factor, 1.0f), col.w);
				};
				...
				ImVec2 descriptionSize =
					ImGui::CalcTextSize(sections[i].description.c_str(),
							    nullptr, true, buttonWidth - 20);
				float buttonHeight = titleSize.y + descriptionSize.y +
						     ImGui::GetStyle().FramePadding.y * 2;

				// Create the button with a custom layout
				if (ImGui::GetScrollMaxY() == 0) {
					if (ImGui::Button("",
							  ImVec2(buttonWidth - 7, buttonHeight))) {
						selected_section = i;
					}
				} else {
					if (ImGui::Button("",
							  ImVec2(buttonWidth - 18, buttonHeight))) {
						selected_section = i;
					}
				}
				...
				if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None)) {
					ImGui::SetDragDropPayload(
						"DND_SECTION", &display_index,
						sizeof(int)); // Dragging by visual index
					ImGui::EndDragDropSource();
				}

				if (ImGui::BeginDragDropTarget()) {
					if (const ImGuiPayload *payload =
						    ImGui::AcceptDragDropPayload("DND_SECTION")) {
						int payload_index = *(const int *)payload->Data;
						std::swap(section_order[payload_index],
							  section_order[display_index]);
					}
				...
				ImVec2 textPos =
					ImVec2(buttonPos.x + ImGui::GetStyle().FramePadding.x,
					       buttonPos.y + ImGui::GetStyle().FramePadding.y);
				ImDrawList *drawList = ImGui::GetWindowDrawList();
				drawList->AddText(textPos,
						  ImGui::ColorConvertFloat4ToU32(
							  GetCurrentTheme().text_primary),
						  sections[i].title.c_str());
				...
				while (ss >> word) {
					std::string potentialLine =
						...
						word;
					ImVec2 potentialLineSize =
						ImGui::CalcTextSize(potentialLine.c_str());
					...
					if (ImGui::GetScrollMaxY() == 0) { // No scrollbar
						...
						if (potentialLineSize.x >
						    buttonWidth - 18) { // Scrollbar
							// Draw the current line and move to the next
							drawList->AddText(
								textPos,
								ImGui::ColorConvertFloat4ToU32(
			...
			ImVec2 rightSectionSize(display_size.x - 23 - left_panel_width,
			...
			if (selected_section >= 0 && selected_section < sections.size()) {
				...
				ImGui::TextWrapped("Settings for %s",
						   sections[selected_section].title.c_str());
				ImGui::Separator();
				ImGui::NewLine();
				...
				if (section_to_key.count(selected_section)) {
					*currentKey = BindKeyMode(currentKey, *currentKey,
								  selected_section);
					ImGui::SetNextItemWidth(170.0f);

					GetKeyNameFromHex(*currentKey, state.keyBufferHuman,
							  sizeof(state.keyBufferHuman));
				}

				// Use the state's keyBuffer for display
				ImGui::InputText("##KeyBufferHuman", state.keyBufferHuman,
						 sizeof(state.keyBufferHuman),
						 ImGuiInputTextFlags_CharsNoBlank |
							 ImGuiInputTextFlags_ReadOnly);
				ImGui::SameLine();
				...
				ImGui::InputText("##KeyBuffer", state.keyBuffer,
						 sizeof(state.keyBuffer),
						 ImGuiInputTextFlags_CharsNoBlank |
							 ImGuiInputTextFlags_CharsHexadecimal |
							 ImGuiInputTextFlags_ReadOnly);
				...
				if (selected_section >= 0 && selected_section < section_amounts) {
					ImGui::Checkbox(("##SectionToggle" +
							 std::to_string(selected_section))
								.c_str(),
							&section_toggles[selected_section]);
				}
				...
				if (selected_section == 0) { // Freeze Macro
					ImGui::TextWrapped(
						"Automatically Unfreeze after this amount of seconds (Anti-Internet-Kick)");
					ImGui::SetNextItemWidth(60.0f);
					ImGui::InputFloat("##FreezeFloat", &maxfreezetime, 0.0f,
							  0.0f, "%.2f");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(300.0f);
					ImGui::SliderFloat("##FreezeSlider", &maxfreezetime, 0.0f,
							   9.8f, "%.2f Seconds");

					char maxfreezeoverrideBuffer[16];
					std::snprintf(maxfreezeoverrideBuffer,
						      sizeof(maxfreezeoverrideBuffer), "%d",
						      maxfreezeoverride);

					ImGui::SetNextItemWidth(50.0f);
					if (ImGui::InputText(
						    "Modify 50ms Default Unfreeze Time (MS)",
						    maxfreezeoverrideBuffer,
						    sizeof(maxfreezeoverrideBuffer),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						maxfreezeoverride =
							std::atoi(maxfreezeoverrideBuffer);
					}

					ImGui::Checkbox(
						"Allow Roblox to be frozen while not tabbed in",
						&freezeoutsideroblox);
					ImGui::Checkbox("Switch from Hold Key to Toggle Key",
							&isfreezeswitch);
					...
					ImGui::Checkbox(
						"Freeze all Found Processes Instead of Newest",
						&takeallprocessids);

					ImGui::SameLine();
					ImGui::TextWrapped(
						"(ONLY EVER USE FOR COMPATIBILITY ISSUES WITH NON-ROBLOX GAMES)");
					ImGui::Separator();
					ImGui::TextWrapped("Explanation:");
					ImGui::NewLine();
					ImGui::TextWrapped(
						"Hold the hotkey to freeze your game, let go of it to release it. Suspending your game also pauses "
				...
				if (selected_section == 1) { // Item Desync
					...
					ImGui::SameLine();
					ImGui::SetNextItemWidth(30.0f);
					ImGui::InputText("##ItemDesync", ItemDesyncSlot,
							 sizeof(ItemDesyncSlot),
							 ImGuiInputTextFlags_CharsDecimal |
								 ImGuiInputTextFlags_CharsNoBlank);
					try {
					...
					ImGui::Separator();
					ImGui::TextWrapped(
						"Equip your item inside of the slot you have picked here, then hold the keybind for 4-7 seconds");
					ImGui::Separator();
					ImGui::TextWrapped("Explanation:");
					ImGui::NewLine();
					ImGui::TextWrapped(
				...
				if (selected_section == 2) { // HHJ
					...
					ImGui::SameLine();
					ImGui::TextWrapped(
						"(EXTREMELY BUGGY/EXPERIMENTAL, WORKS BEST ON HIGH FPS AND SHALLOW ANGLE TO WALL)");
					ImGui::TextWrapped("Length of HHJ flicks (ms):");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(52);
					if (ImGui::InputText(
						    "##HHJLength", HHJLengthChar,
						    sizeof(HHJLengthChar),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
					...
					ImGui::TextWrapped(
						"(ADVANCED) Override current freeze delay (Non Zero) (ms): ");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(52);
					if (ImGui::InputText(
						    "##HHJFreezeDelayOverride",
						    HHJFreezeDelayOverrideChar,
						    sizeof(HHJFreezeDelayOverrideChar),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
							HHJFreezeDelayOverride = std::stoi(
								HHJFreezeDelayOverrideChar);
						} catch (const std::invalid_argument &e) {
						} catch (const std::out_of_range &e) {
						}
					...
					ImGui::TextWrapped(
						"(ADVANCED) Delay after freezing before shiftlock is held (ms): ");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(52);
					if (ImGui::InputText(
						    "##HHJDelay1", HHJDelay1Char,
						    sizeof(HHJDelay1Char),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
					...
					ImGui::TextWrapped(
						"(ADVANCED) Time shiftlock is held before spinning (ms): ");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(52);
					if (ImGui::InputText(
						    "##HHJDelay2", HHJDelay2Char,
						    sizeof(HHJDelay2Char),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
					...
					ImGui::TextWrapped(
						"(ADVANCED) Time shiftlock is held after freezing (ms): ");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(52);
					if (ImGui::InputText(
						    "##HHJDelay3", HHJDelay3Char,
						    sizeof(HHJDelay3Char),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
					...
					ImGui::InvisibleButton(
						"##tooltip",
						ImGui::CalcTextSize("What is this (?)"));
					if (ImGui::IsItemHovered(
						    ImGuiHoveredFlags_AllowWhenDisabled))
						ImGui::SetTooltip(
							"These are advanced options configurable for HHJ.\nThey modify the logic of what occurs after the unfreeze.\n"
					...
					ImGui::SetCursorScreenPos(ImVec2(
						tooltipcursorpos.x +
							ImGui::CalcTextSize("What is this (?)").x,
						tooltipcursorpos.y));
					ImGui::NewLine();

					ImGui::Separator();
					ImGui::TextWrapped(
						"This module abuses Roblox's conversion from angular velocity to regular velocity, and its flawed centre of mass calculation.");
					ImGui::Separator();
					ImGui::TextWrapped("IMPORTANT:");
					ImGui::TextWrapped(
						"Have your Sensitivity and Cam-Fix options set before using this module.");
					ImGui::Separator();
					ImGui::TextWrapped("Explanation:");
					ImGui::NewLine();
					ImGui::TextWrapped(
						"Assuming unequip com offset set to /e dance2 is used prior to offset com, to perform a Helicopter High Jump, "
					...
					ImGui::Separator();
					ImGui::TextWrapped(
						"If you are struggling with the lag high jump timing part, you can try using the \"Automatically time inputs\" feature. "
				...
				if (selected_section == 3) { // Speedglitch
					...
					if (CurrentSensValue != PreviousSensValue) {
						if (camfixtoggle) {
							try {
								RobloxPixelValue = static_cast<int>(
									((500.0f /
									  CurrentSensValue) *
									 (static_cast<float>(359) /
									  360)) +
									0.5f);
							} catch (const std::invalid_argument &e) {
							} catch (const std::out_of_range &e) {
						...
						PreviousSensValue = CurrentSensValue;
						sprintf(RobloxPixelValueChar, "%d",
							RobloxPixelValue);
					}

					ImGui::TextWrapped(
						"Pixel Value for 180 Degree Turn BASED ON SENSITIVITY:");
					ImGui::SetNextItemWidth(90.0f);
					ImGui::SameLine();
					if (ImGui::InputText(
						    "##PixelValue", RobloxPixelValueChar,
						    sizeof(RobloxPixelValueChar),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
							speed_strengthx =
								std::stoi(RobloxPixelValueChar);
							speed_strengthy =
								-std::stoi(RobloxPixelValueChar);
						} catch (const std::invalid_argument &e) {
						} catch (const std::out_of_range &e) {
					...
					ImGui::Checkbox("Switch from Toggle Key to Hold Key",
							&isspeedswitch);
					ImGui::TextWrapped(
						"This module abuses Roblox's conversion from angular velocity to regular velocity, and its flawed centre of mass calculation.");
					ImGui::Separator();
					ImGui::TextWrapped(
						"IMPORTANT: Have your Sensitivity and Cam-Fix options set before using this module.");
					ImGui::Separator();
					ImGui::TextWrapped("Explanation:");
					ImGui::NewLine();

					ImGui::TextWrapped(
						"Assuming unequip \"/e dance2\" is used prior to offset com, to activate a speed glitch, enable shiftlock mode "
				...
				if (selected_section == 4) { // Gear Unequip COM offset
					...
					ImGui::SetNextItemWidth(30.0f);
					if (ImGui::InputText(
						    "##Gearslot", ItemSpeedSlot,
						    sizeof(ItemSpeedSlot),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
					...
					ImGui::TextWrapped(
						"Type in a custom chat message! (Disables gear equipping, just pastes your message in chat)");
					ImGui::TextWrapped(
						"(Leave this blank if you don't want a custom message)");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x);
					ImGui::InputText("##CustomText", CustomTextChar,
							 sizeof(CustomTextChar));

					ImGui::SetNextItemWidth(150.0f);
					if (ImGui::BeginCombo("Select Emote",
							      optionsforoffset[selected_dropdown])) {
						for (int i = 0; i < IM_ARRAYSIZE(optionsforoffset);
						     i++) {
							bool is_selected = (selected_dropdown == i);
							if (ImGui::Selectable(optionsforoffset[i],
									      is_selected)) {
								selected_dropdown =
									i; // Update the selected option
								text = optionsforoffset
									[selected_dropdown];
							}
							if (is_selected) {
					...
					ImGui::AlignTextToFramePadding();
					ImGui::TextWrapped(
						"Key to Press After Message/Emote paste:");
					ImGui::SameLine();

					if (ImGui::Button((enterkeyState.buttonText + "##EnterKey")
								  .c_str())) {
					...
					ImGui::SameLine();
					vk_enterkey = BindKeyMode(&vk_enterkey, vk_enterkey,
								  selected_section);
					ImGui::SetNextItemWidth(150.0f);
					GetKeyNameFromHex(vk_enterkey, enterkeyState.keyBufferHuman,
							  sizeof(enterkeyState.keyBufferHuman));
					ImGui::InputText("Hex:", enterkeyState.keyBufferHuman,
							 sizeof(enterkeyState.keyBufferHuman),
							 ImGuiInputTextFlags_CharsNoBlank |
								 ImGuiInputTextFlags_ReadOnly);
					ImGui::SameLine();
					...
					ImGui::InputText(
						"##HumanKey", enterkeyState.keyBuffer,
						sizeof(enterkeyState.keyBuffer),
						ImGuiInputTextFlags_CharsNoBlank |
							ImGuiInputTextFlags_CharsHexadecimal);
					ImGui::PopID();

					ImGui::Checkbox("Let the macro Keep the item equipped",
							&unequiptoggle);
					ImGui::Checkbox(
						"Make Unequip Com only work while tabbed into Roblox",
						&unequipinroblox);

					ImGui::Separator();
					ImGui::TextWrapped(
						"This module allows you to trick Roblox into thinking your centre of mass is elsewhere. This is used in the Helicopter "
						"High Jump, Speed Glitch and Walless LHJ modules (may change in the future).");
					ImGui::Separator();
					ImGui::TextWrapped(
						"IMPORTANT: This ONLY works in R6. Although the glitch is possible in R15, the macro isn't built around that rig type. "
						"An Item is also required, and must be placed in the corresponding gear slot (3 by default).");
					ImGui::NewLine();
					ImGui::TextWrapped("Usage:");
					ImGui::TextWrapped(
						"Assuming you have a gear ready, to get an offset com, put the gear into the corresponding gear slot (set above), "
				...
				if (selected_section == 5) { // Presskey / Press a Key
					...
					if (ImGui::Button(
						    (dkeyState.buttonText + "##DKey").c_str())) {
					...
					ImGui::SetNextItemWidth(170.0f);
					GetKeyNameFromHex(vk_dkey, dkeyState.keyBufferHuman,
							  sizeof(dkeyState.keyBufferHuman));
					ImGui::InputText("Key to Press", dkeyState.keyBufferHuman,
							 sizeof(dkeyState.keyBufferHuman),
							 ImGuiInputTextFlags_CharsNoBlank |
								 ImGuiInputTextFlags_ReadOnly);
					ImGui::SameLine();
					...
					// Use the state's keyBuffer for hex display
					ImGui::InputText(
						"(Hexadecimal)", dkeyState.keyBuffer,
						sizeof(dkeyState.keyBuffer),
						ImGuiInputTextFlags_CharsNoBlank |
							ImGuiInputTextFlags_CharsHexadecimal);
					ImGui::PopID();
					...
					if (ImGui::InputText(
						    "##PressKeyDelayChar", PressKeyDelayChar,
						    sizeof(PressKeyDelayChar),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
							PressKeyDelay =
								std::stoi(PressKeyDelayChar);
						} catch (const std::invalid_argument &e) {
						} catch (const std::out_of_range &e) {
						}
					}
					...
					ImGui::SetNextItemWidth(80);
					if (ImGui::InputText(
						    "##PressKeyBonusDelayChar",
						    PressKeyBonusDelayChar,
						    sizeof(PressKeyBonusDelayChar),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
							PressKeyBonusDelay =
								std::stoi(PressKeyBonusDelayChar);
						} catch (const std::invalid_argument &e) {
						} catch (const std::out_of_range &e) {
						}
					}
					...
					ImGui::Checkbox(
						"Make PressKey only work while tabbed into Roblox",
						&presskeyinroblox);
					...
					ImGui::NewLine();
					ImGui::TextWrapped(
						"It will press the second keybind for a single frame whenever you press the first keybind. "
				...
				if (selected_section == 6) { // Wallhop
					...
					float sensValue = std::atof(RobloxSensValue);
					if (sensValue != 0.0f) {
						snprintf(WallhopDegrees, sizeof(WallhopDegrees),
							 "%d",
							 static_cast<int>(
								 360 *
								 (std::atof(WallhopPixels) *
								  std::atof(RobloxSensValue)) /
								 (camfixtoggle ? 1000 : 720)));
					}

					if (ImGui::InputText(
						    "##WallhopDegrees", WallhopDegrees,
						    sizeof(WallhopDegrees),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						float pixels =
							std::atof(WallhopDegrees) *
							(camfixtoggle ? 1000.0f : 720.0f) /
							(360.0f * std::atof(RobloxSensValue));
						snprintf(WallhopPixels, sizeof(WallhopPixels),
							 "%.0f", pixels);
						try {
							wallhop_dx = std::round(
								std::stoi(WallhopPixels));
					...
					ImGui::SetNextItemWidth(70.0f);
					if (ImGui::InputText(
						    "##WallhopPixels", WallhopPixels,
						    sizeof(WallhopPixels),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
							wallhop_dx = std::round(
					...
					ImGui::SetNextItemWidth(70.0f);
					if (ImGui::InputText(
						    "##WallhopDelay", WallhopDelayChar,
						    sizeof(WallhopDelayChar),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
							WallhopDelay = std::round(
								std::stoi(WallhopDelayChar));
						} catch (const std::invalid_argument &e) {
						} catch (const std::out_of_range &e) {
						}
					}
					...
					ImGui::TextWrapped(
						"Bonus Wallhop Delay Before Jumping (ms):");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(70.0f);
					if (ImGui::InputText(
						    "##WallhopBonusDelay", WallhopBonusDelayChar,
						    sizeof(WallhopBonusDelayChar),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
							WallhopBonusDelay = std::round(
								std::stoi(WallhopBonusDelayChar));
						} catch (const std::invalid_argument &e) {
						} catch (const std::out_of_range &e) {
						}
					}
					...
					ImGui::Checkbox(
						"Switch to Left-Flick Wallhop",
						&wallhopswitch); // Left Sided wallhop switch
					...
					ImGui::TextWrapped("IMPORTANT:");
					ImGui::TextWrapped(
						"THE ANGLE THAT YOU TURN IS DIRECTLY RELATED TO YOUR ROBLOX SENSITIVITY. "
					...
					ImGui::NewLine();
					ImGui::TextWrapped(
						"This Macro automatically flicks your screen AND jumps at the same time, performing a wallhop.");
				}

				if (selected_section == 7) { // Walless LHJ
					ImGui::Checkbox("Switch to Left-Sided LHJ",
							&wallesslhjswitch); // Left Sided lhj switch
					ImGui::Separator();
					ImGui::TextWrapped("Explanation:");
					ImGui::NewLine();
					ImGui::TextWrapped(
						"If you offset your center of mass to any direction EXCEPT directly upwards, you will be able to perform "
				...
				if (selected_section == 8) { // Item Clip
					...
					ImGui::TextWrapped(
						"Item Clip Delay in Milliseconds (Default 34ms):");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(120.0f);
					if (ImGui::InputText(
						    "##ItemClipDelay", ItemClipDelay,
						    sizeof(ItemClipDelay),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
					...
					ImGui::NewLine();
					ImGui::TextWrapped(
						"This macro will equip and unequip your item in the amount of milliseconds you put in. "
					...
					ImGui::TextWrapped(
						"Also, for convenience sake, you cannot activate item clip unless you're tabbed into roblox.");
				}

				if (selected_section == 9) { // Laugh Clip
					ImGui::Checkbox(
						"Disable S being pressed (Slightly weaker laugh clips, but interferes with movement less)",
						&laughmoveswitch);
					ImGui::TextWrapped("Explanation:");
					ImGui::NewLine();
					ImGui::TextWrapped("MUST BE ABOVE 60 FPS AND IN R6!");
					ImGui::TextWrapped(
						"Go against a wall unshiftlocked and angle your camera DIRECTLY OPPOSITE TO THE WALL. "
				...
				if (selected_section == 10) { // Wall-Walk
					...
					if (CurrentWallWalkValue != PreviousWallWalkValue) {
						if (camfixtoggle) {
							wallwalk_strengthx = static_cast<int>(round(
								(500.0f / CurrentWallWalkValue) *
								0.13f));
							wallwalk_strengthy = -static_cast<int>(
								round((500.0f /
								       CurrentWallWalkValue) *
								      0.13f));
						} else {
							wallwalk_strengthx = static_cast<int>(round(
								(360.0f / CurrentWallWalkValue) *
								0.13f));
							wallwalk_strengthy = -static_cast<int>(
								round((360.0f /
								       CurrentWallWalkValue) *
					...
					ImGui::TextWrapped(
						"Wall-Walk Pixel Value BASED ON SENSITIVITY (meant to be low):");
					ImGui::SetNextItemWidth(90.0f);
					ImGui::SameLine();
					ImGui::InputText("##PixelValue", RobloxWallWalkValueChar,
							 sizeof(RobloxWallWalkValueChar),
							 ImGuiInputTextFlags_CharsDecimal |
								 ImGuiInputTextFlags_CharsNoBlank);

					ImGui::Checkbox("Switch to Left-Flick Wallwalk",
							&wallwalktoggleside);

					ImGui::SetNextItemWidth(100.0f);
					ImGui::InputText(
						"Delay Between Flicks (Don't change from 72720 unless neccessary):",
						RobloxWallWalkValueDelayChar,
						sizeof(RobloxWallWalkValueDelayChar),
						ImGuiInputTextFlags_CharsDecimal |
							ImGuiInputTextFlags_CharsNoBlank);

					try {
						RobloxWallWalkValueDelay =
							atof(RobloxWallWalkValueDelayChar);
					} catch (const std::invalid_argument &e) {
					} catch (const std::out_of_range &e) {
					}
					...
					try { // Error Handling
						wallwalk_strengthx =
							std::stoi(RobloxWallWalkValueChar);
						wallwalk_strengthy =
							-std::stoi(RobloxWallWalkValueChar);
					} catch (const std::invalid_argument &e) {
					} catch (const std::out_of_range &e) {
					...
					ImGui::Separator();
					ImGui::TextWrapped(
						"IMPORTANT: FOR MOST OPTIMAL RESULTS, INPUT YOUR ROBLOX INGAME SENSITIVITY!");
					ImGui::TextWrapped(
						"THE HIGHER FPS YOU ARE, THE MORE STABLE IT GETS, HOWEVER 60 FPS IS ENOUGH FOR INFINITE DISTANCE");
					ImGui::TextWrapped(
					...
					ImGui::NewLine();
					ImGui::TextWrapped(
						"This macro abuses the way leg raycast physics work to permanently keep wallhopping, without jumping "
				...
				if (selected_section == 11) { // Spamkey
					...
					ImGui::SameLine();
					vk_spamkey = BindKeyMode(&vk_spamkey, vk_spamkey,
								 selected_section);
					ImGui::SetNextItemWidth(170.0f);
					GetKeyNameFromHex(vk_spamkey, spamState.keyBufferHuman,
							  sizeof(spamState.keyBufferHuman));
					ImGui::InputText("Key to Press", spamState.keyBufferHuman,
							 sizeof(spamState.keyBufferHuman),
							 ImGuiInputTextFlags_CharsNoBlank |
								 ImGuiInputTextFlags_ReadOnly);
					ImGui::SameLine();
					...
					ImGui::PushID("Press2");
					ImGui::InputText(
						"(Hexadecimal)", spamState.keyBuffer,
						sizeof(spamState.keyBuffer),
						ImGuiInputTextFlags_CharsNoBlank |
							ImGuiInputTextFlags_CharsHexadecimal);
					ImGui::PopID();
					...
					ImGui::TextWrapped(
						"I do not take any responsibility if you set the delay to 0ms");
					ImGui::Checkbox("Switch from Toggle Key to Hold Key",
							&isspamswitch);
					ImGui::Separator();
					ImGui::TextWrapped("Explanation:");
					ImGui::NewLine();
					ImGui::TextWrapped(
						"This macro will spam the second key with a millisecond delay. "
						"This can be used as an autoclicker for any games you want, or a key-spam.");
				}

				if (selected_section == 12) { // Ledge Bounce
					ImGui::Checkbox("Switch Ledge Bounce to Left-Sided",
							&bouncesidetoggle);
					ImGui::Checkbox("Stay Horizontal After Bounce",
							&bouncerealignsideways);
					ImGui::Checkbox("Automatically Hold Movement Keys",
							&bounceautohold);
					ImGui::Separator();
					ImGui::TextWrapped("IMPORTANT:");
					ImGui::TextWrapped(
						"PLEASE SET YOUR ROBLOX SENS AND CAM-FIX CORRECTLY SO IT CAN ACTUALLY DO THE PROPER TURNS! It works best at high FPS (120+).");
					ImGui::TextWrapped(
						"Also, if you set it to automatically hold movement keys, PLEASE HOLD THE KEY AT THE PROPER TIME, else it will keep moving forever.");
					ImGui::Separator();
					ImGui::TextWrapped("Explanation:");
					ImGui::NewLine();
					ImGui::TextWrapped(
				...
				if (selected_section == 13) { // Bunnyhop
					ImGui::TextWrapped(
						"Bunnyhop Delay in Milliseconds (Default 10ms):");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(120.0f);
					if (ImGui::InputText(
						    "##BunnyhopDelay", BunnyHopDelayChar,
						    sizeof(BunnyHopDelayChar),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
						try {
					...
					ImGui::Checkbox("Enable Intelligent Auto-Toggle",
							&bunnyhopsmart);

					ImGui::Separator();
					ImGui::TextWrapped(
						"If Intelligent Auto-Toggle is on, pressing your chat key will temporarily disable bhop "
					...
					ImGui::TextWrapped(
						"This will not be active unless you are currently inside of the target program.");
				}

				if (selected_section == 14) { // Floor Bounce
					ImGui::Checkbox(
						"Attempt to HHJ for potentially slightly more height (EXPERIMENTAL, NOT RECOMMENDED)",
						&floorbouncehhj);
					if (ImGui::CollapsingHeader(
						    "Advanced Floor Bounce HHJ Options",
						    showadvancedhhjbounce
						...
						if (ImGui::InputText(
							    "Delay (Milliseconds) after unfreezing and before shiftlocking",
							    FloorBounceDelay1Char,
							    sizeof(FloorBounceDelay1Char),
							    ImGuiInputTextFlags_CharsDecimal |
								    ImGuiInputTextFlags_CharsNoBlank)) {
							try {
								FloorBounceDelay1 = std::stoi(
									FloorBounceDelay1Char);
							} catch (const std::invalid_argument &e) {
							} catch (const std::out_of_range &e) {
							}
						...
						ImGui::SetNextItemWidth(50);
						if (ImGui::InputText(
							    "Delay (Milliseconds) before enabling helicoptering",
							    FloorBounceDelay2Char,
							    sizeof(FloorBounceDelay2Char),
							    ImGuiInputTextFlags_CharsDecimal |
								    ImGuiInputTextFlags_CharsNoBlank)) {
							try {
								FloorBounceDelay2 = std::stoi(
									FloorBounceDelay2Char);
							} catch (const std::invalid_argument &e) {
							} catch (const std::out_of_range &e) {
							}
						...
						ImGui::SetNextItemWidth(50);
						if (ImGui::InputText(
							    "Time (Milliseconds) spent helicoptering",
							    FloorBounceDelay3Char,
							    sizeof(FloorBounceDelay3Char),
							    ImGuiInputTextFlags_CharsDecimal |
								    ImGuiInputTextFlags_CharsNoBlank)) {
							try {
								FloorBounceDelay3 = std::stoi(
									FloorBounceDelay3Char);
							} catch (const std::invalid_argument &e) {
							} catch (const std::out_of_range &e) {
							}
					...
					ImGui::TextWrapped("IMPORTANT:");
					ImGui::TextWrapped(
						"This module only works at default Roblox gravity. If you can find an equation to allow conversion to different gravities, be my guest.");
					ImGui::TextWrapped("Only works in R6 for now.");
					ImGui::TextWrapped(
						"FPS must be set to 160 or more to function properly. Higher is better.");
					ImGui::Separator();
					ImGui::TextWrapped("Explanation:");
					ImGui::NewLine();
					ImGui::TextWrapped(
				...
				if (selected_section == 15) { // Lag Switch
					...
					ImGui::Checkbox("Prevent Roblox Disconnection (",
							&prevent_disconnect);
					...
					ImGui::Text(")");
					ImGui::SetCursorScreenPos(tooltipcursorpos);
					ImVec2 TextSizeCalc = ImGui::CalcTextSize(
						"Prevent Roblox Disconnection (?)      ");
					ImGui::InvisibleButton("##tooltip", TextSizeCalc);
					if (ImGui::IsItemHovered(
						    ImGuiHoveredFlags_AllowWhenDisabled))
						ImGui::SetTooltip(
					...
					ImGui::SetCursorScreenPos(
						ImVec2(tooltipcursorpos.x, tooltipcursorpos.y + 6));
					ImGui::NewLine();

					bool filter_changed = false;

					if (ImGui::Checkbox("Only Lag Switch Roblox",
							    &lagswitchtargetroblox)) {
						filter_changed = true;
					}

					if (ImGui::IsItemHovered()) {
						ImGui::SetTooltip(
							"Filters only Roblox traffic. Uncheck this AND Only Lag Switch Roblox to block EVERYTHING.");
					}

					if (ImGui::Checkbox("Also Block TCP (Websites)",
							    &lagswitchusetcp)) {
						filter_changed = true;
					}
					...
					ImGui::Checkbox(
						"Auto-Unlag (Anti-Kick) (Non-Roblox Games Only)",
						&lagswitch_autounblock);

					// Disable inputs if toggle is off
					if (!lagswitch_autounblock)
						ImGui::BeginDisabled();

					ImGui::TextWrapped(
						"Automatically stops lagging after this amount of seconds");

					ImGui::SetNextItemWidth(60.0f);
					ImGui::InputFloat("##LagFloat", &lagswitch_max_duration,
							  0.0f, 0.0f, "%.2f");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(300.0f);
					ImGui::SliderFloat("##LagSlider", &lagswitch_max_duration,
							   0.0f, 15.0f, "%.2f Seconds");

					char lagUnblockBuffer[16];
					std::snprintf(lagUnblockBuffer, sizeof(lagUnblockBuffer),
						      "%d", lagswitch_unblock_ms);

					ImGui::SetNextItemWidth(50.0f);
					if (ImGui::InputText(
						    "Modify 50ms Default Unlag Time (MS)",
						    lagUnblockBuffer, sizeof(lagUnblockBuffer),
						    ImGuiInputTextFlags_CharsDecimal |
							    ImGuiInputTextFlags_CharsNoBlank)) {
					...
					ImGui::Separator();
					if (ImGui::Checkbox(
						    "Block Outbound (Upload/Send) (Players won't be able to see you move)",
						    &lagswitchoutbound))
						filter_changed = true;
					if (ImGui::Checkbox(
						    "Block Inbound (Download/Recv) (You won't be able to see other players move)",
						    &lagswitchinbound))
						filter_changed = true;
					...
					if (!lagswitchoutbound && !lagswitchinbound) {
						...
						ImGui::TextWrapped(
							"WARNING: Both Inbound and Outbound are unchecked.\nThe Lag Switch will not block any packets.");
						ImGui::PopStyleColor();
					}
					...
					if (ImGui::Checkbox("Fake Lag (Simulate High Ping)",
							    &lagswitchlag)) {
					...
					ImGui::InputInt("##LagDelayInput", &lagswitchlagdelay, 10,
							100);

					// Direction Toggles for Fake Lag
					if (ImGui::Checkbox("Lag Inbound (Recv)##FakeLag",
							    &lagswitchlaginbound))
						filter_changed = true;
					ImGui::SameLine();
					if (ImGui::Checkbox("Lag Outbound (Send)##FakeLag",
							    &lagswitchlagoutbound))
						filter_changed = true;

					// Warning if enabled but no directions selected
					if (lagswitchlag && !lagswitchlaginbound &&
					    !lagswitchlagoutbound) {
						ImGui::TextColored(
							GetCurrentTheme().warning_color,
							"Select at least one direction to lag!");
					}

					if (lagswitchlag &&
					    ((lagswitchoutbound && lagswitchlagoutbound) ||
					     (lagswitchinbound && lagswitchlaginbound))) {
						ImGui::PushStyleColor(
							ImGuiCol_Text,
							GetCurrentTheme().warning_color);
						ImGui::TextWrapped(
							"WARNING: Both Blocking and Lagging are enabled for Inbound/Outbound packets.\nBlocking takes priority over lagging.");
						ImGui::PopStyleColor();
					}
					...
					ImGui::Checkbox("Hide When Not Actively Lagswitching",
							&overlay_hide_inactive);
					...
					if (!overlay_use_bg)
						ImGui::BeginDisabled();
					float colors[3] = {overlay_bg_r, overlay_bg_g,
							   overlay_bg_b};
					...
					if (!g_isLinuxWine) {
						...
						if (ImGui::Button(bWinDivertEnabled
									  ? "Disable WinDivert"
									  : "Enable WinDivert")) {
							if (!bWinDivertEnabled) {
								if (IsRunAsAdmin()) {
									if (TryLoadWinDivert()) {
										...
										g_windivert_running =
											true;
										WinDivertThread = std::thread(
											WindivertWorkerThread);
									} else {
										std::cerr
											<< "Failed to load WinDivert files."
											<< std::endl;
									}
								} else {
								...
								g_windivert_blocking =
									false; // Reset blocking state
								SafeCloseWinDivert(); // Kill the thread loop
								if (WinDivertThread.joinable())
									WinDivertThread.join();
							}
						}
						...
						ImGui::TextColored(
							bWinDivertEnabled
								? GetCurrentTheme().success_color
								: GetCurrentTheme().error_color,
							bWinDivertEnabled ? "Driver Running"
									  : "Driver Not Running");
					}
			...
			if (ImGui::BeginChild(
				    "BottomControls", ImVec2(childSize.x - 1, 30),
				    false, // 'false' here removes the border from BottomControls
				...
				if (ImGui::Checkbox("##OnTopToggle", &ontoptoggle)) {
					SetWindowPos(hwnd,
						     ontoptoggle ? HWND_TOPMOST : HWND_NOTOPMOST, 0,
				...
				// Adjust text starting point by 2 pixels downwards
				ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 3);
				if (ImGui::SliderFloat("##OpacitySlider", &windowOpacityPercent,
						       20.0f, 100.0f, "%.0f%%")) {
					BYTE alpha = static_cast<BYTE>(
						(windowOpacityPercent / 100.0f) * 255);
				...
				draw_list->AddRectFilled(
					ImVec2(childPos.x + childSize.x - rounding,
					       childPos.y + childSize.y - rounding),
				...
				draw_list->AddLine(ImVec2(childPos.x + childSize.x - 1,
							  childPos.y + childSize.y - rounding - 1),
						   ImVec2(childPos.x + childSize.x - 1,
							  childPos.y + childSize.y + 29),
						   border_color);
				...
				draw_list->AddLine(
					...
					ImVec2(childPos.x + childSize.x - 1,
					       childPos.y + childSize.y - 1),
					bg_color, // Use BG color to "erase" the existing border
					1.0f);

				// Draw the Bottom Border
				draw_list->AddLine(ImVec2(childPos.x,
							  childPos.y + childSize.y + 29),
						   ImVec2(childPos.x + childSize.x,
							  childPos.y + childSize.y + 29),
						   border_color, 1.0f);
...
static void UpdateSMCThread()
{
	...
	if (!remoteVersion.empty()) {
		...
		if (remoteVersion != localVersion) {
			...
			int result = MessageBoxW(NULL, message.c_str(), L"Update Available",
		    ...
		    _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	char debugbuffer[16];
	if (GetEnvironmentVariableA("DEBUG", debugbuffer, sizeof(debugbuffer)) &&
	    std::string(debugbuffer) == "1") {
		CreateDebugConsole();
	} else {
	...
	hProcess = GetProcessHandles_Compat(targetPIDs, PROCESS_SUSPEND_RESUME |
								PROCESS_QUERY_LIMITED_INFORMATION);
	std::vector<HWND> rbxhwnd =
		FindWindowByProcessHandle(hProcess); // SET ROBLOX WINDOW HWND RAHHHHH
	...
	while (!done) {
		bool tabbedintoroblox = IsForegroundWindowProcess(hProcess);
		{
			...
			if ((macrotoggled && notbinding && section_toggles[0])) {
				...
				if (isfreezeswitch) { // Toggle mode
					if (isMButtonPressed && !wasMButtonPressed &&
					    (freezeoutsideroblox ||
					     tabbedintoroblox)) { // Detect button press edge
						isSuspended =
							!isSuspended; // Toggle the freeze state
						SuspendOrResumeProcesses_Compat(
							targetPIDs, hProcess, isSuspended);
					...
					if (isMButtonPressed &&
					    (freezeoutsideroblox || tabbedintoroblox)) {
						if (!isSuspended) {
							SuspendOrResumeProcesses_Compat(
								targetPIDs, hProcess,
								true); // Freeze on hold
				...
				if (isSuspended) {
					...
					if (elapsed >= (maxfreezetime * 1000)) {
						// Unsuspend for 50 ms
						SuspendOrResumeProcesses_Compat(targetPIDs,
										hProcess, false);
			...
			if (IsHotkeyPressed(vk_f5) && tabbedintoroblox && macrotoggled &&
			    notbinding && section_toggles[1]) {
				if (!isdesync) {
			...
			// PressKey
			if (IsHotkeyPressed(vk_zkey) && macrotoggled && notbinding &&
			    section_toggles[5] && (!presskeyinroblox || tabbedintoroblox)) {
				if (!ispresskey) {
			...
			if (IsHotkeyPressed(vk_xbutton2) && macrotoggled && notbinding &&
			    section_toggles[6]) {
				if (!iswallhop) {
			...
			if (IsHotkeyPressed(vk_f6) && macrotoggled && notbinding &&
			    section_toggles[7]) {
				if (!islhj) {
					if (wallesslhjswitch) {
						HoldKey(0x1E);
					...
					if (!globalzoomin) {
						...
						HoldKeyBinded(globalzoominreverse
								      ? VK_MOUSE_WHEEL_DOWN
								      : VK_MOUSE_WHEEL_UP);
					}
					SuspendOrResumeProcesses_Compat(targetPIDs, hProcess,
									false);
					std::this_thread::sleep_for(std::chrono::milliseconds(50));
					if (!globalzoomin) {
			...
			// Speedglitch
			if (IsHotkeyPressed(vk_xkey) && tabbedintoroblox && macrotoggled &&
			    notbinding && section_toggles[3]) {
				if (!isspeedglitch) {
					isspeed = !isspeed;
					isspeedglitch = true;
				}
			...
			if (IsHotkeyPressed(vk_f8) && macrotoggled && notbinding &&
			    section_toggles[4] && (!unequipinroblox || tabbedintoroblox)) {
				if (!isunequipspeed) {
					if (chatoverride) {
						HoldKey(0x35);
					} else {
			...
			// Helicopter High jump
			if (IsHotkeyPressed(vk_xbutton1) && macrotoggled && notbinding &&
			    section_toggles[2]) {
				if (!HHJ) {
					...
					if (strcmp(HHJFreezeDelayOverrideChar, "0") == 0 ||
					    strcmp(HHJFreezeDelayOverrideChar, "") == 0) {
					...
					std::this_thread::sleep_for(
						std::chrono::milliseconds(HHJDelay1));
					if (!globalzoomin) {
						HoldKeyBinded(vk_shiftkey);
					} else {
						HoldKeyBinded(globalzoominreverse
					...
					std::this_thread::sleep_for(
						std::chrono::milliseconds(HHJDelay2));
					isHHJ.store(true, std::memory_order_relaxed);
					std::this_thread::sleep_for(
						std::chrono::milliseconds(HHJDelay3));
					if (!globalzoomin) {
						ReleaseKeyBinded(vk_shiftkey);
					}
					std::this_thread::sleep_for(
						std::chrono::milliseconds(HHJLength));
			...
			// Spamkey
			if (IsHotkeyPressed(vk_leftbracket) && macrotoggled && notbinding &&
			    section_toggles[11]) {
				if (!isspam) {
			...
			// Laughkey
			if (IsHotkeyPressed(vk_laughkey) && tabbedintoroblox && macrotoggled &&
			    notbinding && section_toggles[9]) {
				if (!islaugh) {
					if (chatoverride) {
						HoldKey(0x35);
					} else {
			...
			// Ledge Bounce
			if (IsHotkeyPressed(vk_bouncekey) && tabbedintoroblox && macrotoggled &&
			    notbinding && section_toggles[12]) {
				if (!isbounce) {
					int turn90 =
						(camfixtoggle ? 250 : 180) / atof(RobloxSensValue);
					int skey = 0x1F; // S key
					int dkey = 0x20; // D key
					int wkey = 0x11; // W key
					...
					if (bouncerealignsideways) {
						MoveMouse(turn90,
							  0); // Turn Right to face towards ledge
						std::this_thread::sleep_for(
			...
			// Item Clip
			if (IsHotkeyPressed(vk_clipkey) && tabbedintoroblox && macrotoggled &&
			    notbinding && section_toggles[8]) {
				if (!isclip) {
			...
			// WallWalk
			if (IsHotkeyPressed(vk_wallkey) && tabbedintoroblox && macrotoggled &&
			    notbinding && section_toggles[10]) {
				if (!iswallwalk) {
			...
			bool can_process_bhop = tabbedintoroblox && section_toggles[13] &&
						macrotoggled && notbinding;
			...
			if (can_process_bhop) {
				bool is_bunnyhop_key_considered_held =
					g_isVk_BunnyhopHeldDown.load(std::memory_order_relaxed) ||
					IsHotkeyPressed(vk_bunnyhopkey);
			...
			// Floor Bounce
			if (IsHotkeyPressed(vk_floorbouncekey) && tabbedintoroblox &&
			    macrotoggled && notbinding && section_toggles[14]) {
				if (!isfloorbounce) {
				...
				if (bWinDivertEnabled) {
					...
					if (islagswitchswitch) {
						...
						if (isPressed && !wasLagSwitchPressed) {
							SetLagswitchEnabled(
								!g_windivert_blocking
									 .load()); // Flip state via compat layer
						}
			...
			if (++counter % 100 == 0) { // Check time every 100th iteration
				...
				if (std::chrono::duration_cast<std::chrono::seconds>(
					    now - processchecktime)
					    .count() >= 1) {

					targetPIDs = FindProcessIdsByName_Compat(settingsBuffer,
										 takeallprocessids);
					...
					hProcess = GetProcessHandles_Compat(
						targetPIDs,
				...
				if (processFound && antiafktoggle && isafk && !g_isLinuxWine) {
					...
					if (elapsedMinutes >= AntiAFKTime) {
						...
						if (!IsForegroundWindowProcess(hProcess)) {
							...
							HWND windowhwnd =
								FindNewestProcessWindow(rbxhwnd);
							SetWindowPos(windowhwnd, HWND_TOPMOST, 0, 0,
								     0, 0, SWP_NOMOVE | SWP_NOSIZE);
							SetWindowPos(windowhwnd, HWND_NOTOPMOST, 0,
								     0, 0, 0,
								     SWP_NOMOVE | SWP_NOSIZE);

							// Get roblox rectangle coords
							RECT robloxRect;
							if (!GetWindowRect(windowhwnd,
									   &robloxRect)) {
								std::cerr
							...
							int centerX = robloxRect.left +
								      (robloxRect.right -
								       robloxRect.left) /
									      2;
							int centerY = robloxRect.top +
								      (robloxRect.bottom -
								       robloxRect.top) /
									      2;
							...
							Sleep(200);
							SetWindowPos(originalHwnd, HWND_TOPMOST, 0,
								     0, 0, 0,
								     SWP_NOMOVE | SWP_NOSIZE);
							SetWindowPos(originalHwnd, HWND_NOTOPMOST,
								     0, 0, 0, 0,
								     SWP_NOMOVE | SWP_NOSIZE);
							Sleep(50);
							...
							SetCursorPos(originalMousePos.x,
								     originalMousePos.y);
		...
		std::this_thread::sleep_for(std::chrono::microseconds(
			50)); // Delay between main code loop (so your cpu doesn't die instantly)
	}
	...
	if (IsRunAsAdmin()) {
		quiet_system(
			"sc stop WinDivert >nul 2>&1"); // Remove windivert service upon closing the app
		quiet_system(
			"sc delete WinDivert >nul 2>&1"); // Remove windivert service upon closing the app
		quiet_system(
			"sc query WinDivert >nul 2>&1"); // Query WinDivert to update windows about its status
	}
