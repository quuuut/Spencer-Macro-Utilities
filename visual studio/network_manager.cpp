...
bool TryLoadWinDivert()
{
	...
	if (!ExtractResource(IDR_SMC_WINDIVERT_SYS1, "SMC_WINDIVERT_SYS", SYS_NAME))
		MessageBoxW(NULL, buffer, L"WinDivert SYS Placement Error", MB_ICONERROR | MB_OK);

	// 2. Extract DLL
	if (!ExtractResource(IDR_SMC_WINDIVERT_DLL1, "SMC_WINDIVERT_DLL", DLL_NAME))
		MessageBoxW(NULL, buffer, L"SMCWinDivert DLL Placement Error",
			    MB_ICONERROR | MB_OK);
	...
	if (!hWinDivertDll) {
		...
		wsprintfW(msg,
			  L"Failed to load SMCWinDivert.dll\n\n"
	...
	pWinDivertHelperParsePacket = (tWinDivertHelperParsePacket)GetProcAddress(
		hWinDivertDll, "WinDivertHelperParsePacket");
	pWinDivertHelperCalcChecksums = (tWinDivertHelperCalcChecksums)GetProcAddress(
		hWinDivertDll, "WinDivertHelperCalcChecksums");
	...
	if (!allLoaded) {
		wsprintfW(buffer, L"Failed to resolve required WinDivert function(s):\n\n%s",
			  missingFuncs);
		MessageBoxW(NULL, buffer, L"WinDivert Symbol Resolution Error",
			    MB_ICONERROR | MB_OK);
	...
	HANDLE testHandle =
		pWinDivertOpen("false", WINDIVERT_LAYER_NETWORK, 0, WINDIVERT_FLAG_SNIFF);
	if (testHandle == INVALID_HANDLE_VALUE) {
		...
		wchar_t msg[1024];
		wsprintfW(msg,
			  L"WinDivert DLL loaded successfully, but driver failed to open.\n\n"
...
bool IsValidRobloxIP(const std::string &ip_str)
{
	if (ip_str.empty())
		return false;
	// 127.x.x.x (Localhost)
	if (ip_str.rfind("127.", 0) == 0)
		return false;
	// 10.x.x.x (Private Class A)
	if (ip_str.rfind("10.", 0) == 0)
		return false;
	// 192.168.x.x (Private Class C)
	if (ip_str.rfind("192.168.", 0) == 0)
		return false;
	// 172.16.x.x - 172.31.x.x (Private Class B)
	if (ip_str.rfind("172.", 0) == 0)
		return false;
	// 0.0.0.0
	if (ip_str == "0.0.0.0")
		return false;
...
void DelaySenderWorker()
{
	while (g_delay_thread_running) {
		...
		if (g_delayed_packet_queue.empty()) {
			g_delay_cv.wait(lock, [] {
				return !g_delayed_packet_queue.empty() || !g_delay_thread_running;
			});
		}

		if (!g_delay_thread_running)
			break;
		...
		if (now >= front_packet.send_time) {
			// It is time to send
			{
				...
				if (hWindivert != INVALID_HANDLE_VALUE && pWinDivertSend) {
					pWinDivertSend(hWindivert, front_packet.data.data(),
						       front_packet.len, NULL, &front_packet.addr);
				}
			}
...
void RobloxLogScannerThread()
{
	...
	if (!g_isLinuxWine) {
		if (SHGetFolderPathA(NULL, CSIDL_LOCAL_APPDATA, NULL, 0, localAppData) == S_OK) {
			...
			std::cerr << "Could not find LocalAppData for Logzz configuration."
				  << std::endl;
			return;
		}
	} else {
		...
		if (!home.empty()) {
			std::string sober_storage =
				home +
				"/.var/app/org.vinegarhq.Sober/data/sober/appData/LocalStorage";
			std::string sober_logs =
				home + "/.var/app/org.vinegarhq.Sober/data/sober/sober_logs";

			// Convert Unix paths to Wine paths using winepath
			...
			std::string wine_storage = convert_to_wine_path(sober_storage);
			std::string wine_logs = convert_to_wine_path(sober_logs);

			if (wine_storage.empty() || wine_logs.empty()) {
				std::cerr
					<< "Failed to convert paths using winepath. Falling back to manual conversion."
					<< std::endl;

				wine_storage = "Z:" + sober_storage;
				wine_logs = "Z:" + sober_logs;
				std::replace(wine_storage.begin(), wine_storage.end(), '/', '\\');
				std::replace(wine_logs.begin(), wine_logs.end(), '/', '\\');
			}

			logzz::local_storage_folder_path = wine_storage;
			logzz::logs_folder_path = wine_logs;

			logzz::load_user_info();
		} else {
			std::cerr << "Could not find HOME directory for Sober configuration."
				  << std::endl;
			return;
		}
	}
		...
		if (s == IN_GAME) {
			...
			auto process_ip = [&](const std::string &ip, const std::string &label) {
				...
				if (g_roblox_dynamic_ips.find(ip) == g_roblox_dynamic_ips.end()) {
					...
					new_ip_found = true;
					std::cout << "Found Roblox Server (" << label << "): " << ip
						  << std::endl;
				}
			};
...
void WindivertWorkerThread()
{
	...
	while (g_windivert_running) {
		...
		if (capture_inbound && capture_outbound)
			direction_filter = "(inbound or outbound)";
		else if (capture_inbound)
			direction_filter = "inbound";
		else if (capture_outbound)
		...
		if (lagswitchtargetroblox) {
			std::string combined_ip_filter = ROBLOX_RANGE_FILTER;
			{
				...
				for (const auto &ip : g_roblox_dynamic_ips) {
					combined_ip_filter += " or (ip.SrcAddr == " + ip +
							      " or ip.DstAddr == " + ip + ")";
				}
			}
		...
		if (lagswitchusetcp)
			final_filter = final_filter + " and (udp or tcp)";
		else
...

		{
			std::lock_guard<std::mutex> lock(g_windivert_handle_mutex);
			hWindivert =
				pWinDivertOpen(final_filter.c_str(), WINDIVERT_LAYER_NETWORK, 0, 0);
		}

		if (hWindivert == INVALID_HANDLE_VALUE) {
			// Handle error / reset IPs if parameter invalid
			if (GetLastError() == ERROR_INVALID_PARAMETER &&
			    !g_roblox_dynamic_ips.empty()) {
				std::unique_lock lock(g_ip_mutex);
		...
		// --- INNER LOOP ---
		while (g_windivert_running) {
			if (!pWinDivertRecv(hWindivert, packet.get(), WINDIVERT_MTU_MAX, &packetLen,
					    &addr)) {
				break;
			}
			...
			if (g_windivert_blocking) {
				...
				if (addr.Outbound && lagswitchoutbound)
					should_hard_block = true;
				if (!addr.Outbound && lagswitchinbound)
					should_hard_block = true;
				...
				if (should_hard_block) {
					if (prevent_disconnect) {
						...
						if (is_rcc)
							continue; // Drop if not pulsing

						// Smart Drop Logic
						bool drop_smart = false;
						if (pWinDivertHelperParsePacket(
							    packet.get(), packetLen, &ip_header,
							    &ipv6_header, &protocol, &icmp_header,
							    &icmpv6_header, &tcp_header,
						...
						if (drop_smart)
							...
							pWinDivertSend(hWindivert, packet.get(),
								       packetLen, NULL,
								       &addr); // Send Heartbeat
							continue;
						}
				...
				if (lagswitchlag) {
					...
					if (addr.Outbound && lagswitchlagoutbound)
						should_delay = true;
					if (!addr.Outbound && lagswitchlaginbound)
						should_delay = true;

					if (should_delay && lagswitchlagdelay > 0) {
						DelayedPacket p;
						p.data.assign(packet.get(),
							      packet.get() + packetLen);
						p.len = packetLen;
						p.addr = addr;
...

						{
							std::lock_guard<std::mutex> qLock(
								g_delay_queue_mutex);
							g_delayed_packet_queue.push_back(
								std::move(p));
						}
